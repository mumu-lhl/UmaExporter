...
class SearchController:
    ...
    def _apply_search_thumbnail_batch(self, prefix, batch_results):
        """Main-thread texture binding for scene and prop thumbnails."""
        for img_id, data in batch_results:
            self.app.texture_registry.replace(prefix, img_id, data, 100, 100)
    ...
    def process_lazy_thumbnails(self):
        ...
        if active_tab == "character_tab":
            ...
            if self.character_thumbnails.process_visible("character_outfits", 16):
                return
        ...
        if to_load_batch:
            ...
            self.app.thumbnail_service.load_search_thumbnails_batch_async(
                tasks,
                lambda results: self._apply_search_thumbnail_batch(
                    active_prefix, results
                ),
            )
